...
4. **Follow conventional commits.** The PR title becomes the merge commit's subject, so follow the [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/#specification) standard. PRs are merged with merge commits rather than squashed, so the commits on your branch land on `main` as well: keep them to discrete semantic changes, and give each one a short title and a body explaining why.
