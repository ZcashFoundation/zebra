...
Re-run the failed job first. release-plz is idempotent for already-published crates and existing tags. The scenarios below need manual steps.

## A crate is on crates.io but no tag was created

On re-run, release-plz sees the crate as published and skips it. Detect by comparing `cargo info <crate>` against `git ls-remote --tags origin`. Recover with:
