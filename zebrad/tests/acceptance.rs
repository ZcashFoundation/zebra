...
//! $ cargo test lightwalletd_sync_update -- --nocapture
//!
//! $ export ZEBRA_TEST_LIGHTWALLETD=true
//! $ export ZEBRA_CACHE_DIR="/path/to/zebra/state"
//! $ cargo test lightwalletd_sync_full -- --ignored --nocapture
//!
//! $ export ZEBRA_TEST_LIGHTWALLETD=true
...
//! $ export ZEBRA_TEST_LIGHTWALLETD=true
//! $ export ZEBRA_CACHE_DIR="/path/to/zebra/state"
//! $ cargo test rpc_fully_synced_test -- --ignored --nocapture
//!
//! $ export ZEBRA_TEST_LIGHTWALLETD=true
//! $ export ZEBRA_CACHE_DIR="/path/to/zebra/state"
//! $ export LWD_CACHE_DIR="/path/to/lightwalletd/database"
//! $ cargo test lightwalletd_send_transactions --features lightwalletd-grpc-tests -- --ignored --nocapture
//!
//! $ export ZEBRA_TEST_LIGHTWALLETD=true
//! $ export ZEBRA_CACHE_DIR="/path/to/zebra/state"
//! $ export LWD_CACHE_DIR="/path/to/lightwalletd/database"
//! $ cargo test lightwalletd_grpc_wallet --features lightwalletd-grpc-tests -- --ignored --nocapture
//! ```
//!
...
// drives populated by the sync_to_checkpoint tests, snapshot those drives,
...
async fn rpc_fully_synced_test() -> Result<()> {
    ...
    let (mut zebrad, zebra_rpc_address) = if let Some(zebrad_and_address) =
        spawn_zebrad_for_rpc(network, "rpc_fully_synced_test", test_type, false)?
    {
...
///
/// This test name must have the same prefix as the `rpc_fully_synced_test`, so they can be run in the same test job.
#[tokio::test]
#[ignore]
