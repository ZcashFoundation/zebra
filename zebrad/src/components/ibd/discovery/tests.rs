...
fn extend_grows_the_range_and_serves_hashes_by_height() {
    ...
    let (feed, mut source) = DiscoverySource::new(base, hash(99));
...
fn extend_skips_an_overlapping_first_hash() {
    ...
    let (feed, mut source) = DiscoverySource::new(base, hash(9));
...
fn release_below_advances_the_base_and_anchor() {
    ...
    let (feed, mut source) = DiscoverySource::new(base, hash(4));
...
fn invalidate_above_truncates_the_range() {
    let base = block::Height(0);
    let (feed, mut source) = DiscoverySource::new(base, hash(255));

    feed.extend(vec![hash(1), hash(2), hash(3), hash(4)]);
    drain(&mut source);

    source.invalidate_above(block::Height(1));

    assert_eq!(source.max_height(), block::Height(1));
    assert_eq!(source.hash(block::Height(1)).unwrap(), Some(hash(2)));
    assert_eq!(source.hash(block::Height(2)).unwrap(), None);
}
...
async fn wait_for_growth_reports_queued_and_awaited_extends() {
    ...
    // Nothing queued: pending until the feed extends.
    source.release_below(block::Height(1));
...
async fn a_finish_behind_queued_hashes_still_reports_growth() {
    ...
    assert!(source.wait_for_growth().await);
    assert!(source.is_final());
    assert_eq!(source.hash(block::Height(0)).unwrap(), Some(hash(1)));
}

#[tokio::test]
async fn wants_more_hashes_follows_the_low_water_mark() {
    let base = block::Height(0);
    ...
    // Fill the source well past the low-water mark.
    ...
    // Committing (releasing) down to the mark asks for more.
    source.release_below(block::Height(50));
    feed.wants_more_hashes().await;
