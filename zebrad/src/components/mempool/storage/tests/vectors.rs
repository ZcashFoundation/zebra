...
fn mempool_storage_crud_exact_mainnet() {
    ...
    let _ = storage.insert(unmined_tx.clone(), Vec::new(), None, None);
...
fn mempool_storage_basic_for_network(network: Network) -> Result<()> {
    ...
    for unmined_transaction in unmined_transactions.clone() {
        let result = storage.insert(unmined_transaction.clone(), Vec::new(), None, None);
        match result {
            Ok(_) => {
...
fn mempool_storage_crud_same_effects_mainnet() {
    ...
    // Insert unmined tx into the mempool.
    let _ = storage.insert(unmined_tx_1.clone(), Vec::new(), None, None);
    ...
    assert_eq!(
        storage.insert(unmined_tx_1, Vec::new(), None, None),
    ...
    // Insert unmined tx into the mempool.
    assert_eq!(
        storage.insert(unmined_tx_2.clone(), Vec::new(), None, None),
    ...
    assert_eq!(
        storage.insert(unmined_tx_2, Vec::new(), None, None),
...
fn mempool_removes_dependent_transactions() -> Result<()> {
    ...
    for unmined_tx in unmined_txs_with_transparent_outputs() {
        ...
        storage
            .insert(unmined_tx.clone(), fake_spent_outpoints, None, None)
