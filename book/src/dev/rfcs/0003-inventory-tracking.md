...
This RFC has been fully implemented with enhancements. The following notes
...
| Component | Location |
|-----------|----------|
| `InventoryRegistry` | `zebra-network/src/peer_set/inventory_registry.rs` |
| `InventoryChange` | `zebra-network/src/peer_set/inventory_registry.rs:81` |
| Routing logic | `zebra-network/src/peer_set/set.rs:906-979` |
| P2C selection | `zebra-network/src/peer_set/set.rs:801-847` |

## Data Structure Evolution

- Uses `IndexMap<InventoryHash, IndexMap<PeerSocketAddr, InventoryMarker>>` instead of
  `HashMap<HashSet<>>` for better memory efficiency and insertion-order preservation
- Tracks both **available** AND **missing** inventory (enhancement over RFC)

## Security Enhancements

- `MAX_INV_PER_MAP = 1000` - limits hashes tracked per inventory type
- `MAX_PEERS_PER_INV = 70` - limits peers tracked per hash
- Prevents DoS attacks like invdos.net

## Rotation Interval

| Parameter | RFC Suggested | Implemented | Location |
|-----------|---------------|-------------|----------|
| Rotation interval | 75 seconds | 53 seconds | `zebra-network/src/constants.rs:153` |

The 53-second interval is tuned to Zcash's block target spacing.

## Three-Tier Fallback (Enhancement)

The RFC proposed binary routing (advertising peers → P2C). Implementation uses:

1. Try advertising peers
2. Try non-missing peers
3. Fail with `PeerError::NotFoundRegistry`

This prevents wasting bandwidth on peers known to be missing inventory.
