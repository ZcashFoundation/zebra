# Dependent Verification and State Updates

- Feature Name: constraint_verification
...
* We defer data dependencies by assuming that unknown inputs are correct, and
  then checking those assumptions before the block is committed to the state.
...
By parallelising chain tip verification, and keeping recent chains in memory, we
...
Blockchain:
* **best chain:** The chain fork with the greatest difficulty is called the best
                  chain. This chain represents the consensus state of the Zcash
...
* **Structural Verification:** See parallel verification.
* **Semantic Verification:** See parallel verification.
* **Constraint Checks:** TODO
* **In-Memory Chain Fork Updates:** An updated chain fork context is created for each
  block, based on the previous block's chain fork context.
* **Chain Tip Updates:** The set of chain tips is updated as each block is
  verified. The best chain tip is updated based on the consensus rules.
* **Best Chain Disk Updates:** When a best chain block is behind the best
  chain's tip by more than the reorganisation limit (100 blocks), it is stored
  to disk. The on-disk state is updated based on that block's chain fork context.
  The in-memory block and chain fork context are dropped to reclaim resources.
* **Chain Fork Pruning:** When a chain fork is behind the best chain's tip by more than
  the pruning limit (100 blocks in Zebra, 288 blocks in zcashd), its in-memory
  blocks and chain fork contexts are dropped to reclaim resources.
...
* **Constraint Verification:**
  * `zebra_consensus::ConstraintVerifier`: Provides a verifier service that
    accepts a `VerificationConstraints, previous_context: ChainForkContext`
    request, performs constraint checks on the `Block` and `ChainForkContext`, and
...
* **Contextual Verification:**
  * `zebra_consensus::ContextualVerifier`: Provides a verifier service that
    accepts a `Block, previous_context: ChainForkContext` request, performs any
    remaining contextual verification on the block, and responds with an
    identifier for the block.
* **In-Memory Chain Updates:**
  * `zebra_consensus::RecentChainForkUpdater`: Provides an updater service that
    accepts a `Block, previous_context: ChainForkContext` request, generates an
    updated chain fork context, dropping references to any chain fork blocks that are
    no longer required to verify subsequent blocks. The service responds with
    an updated `ChainForkContext`, which contains a reference to the block itself.
* **Chain Tip Updates:**
  * `zebra_consensus::ChainTipUpdater`: Provides an updater service that
    accepts a `ChainForkContext, Arc<Mutex<ChainTips>>>` request, updates
    the set of chain tips, and updates the best chain tip, if required.
    (New blocks on side-chains might not lead to a best chain tip update.)
    The `ChainForkContext` becomes one of the new tips in the list.
...
* **Best Chain Disk Updates:**
  * `zebra_consensus::MainChainDiskUpdater`: Provides an updater service that
    ...
    service updates the disk state based on the earliest chain fork contexts in the
    best chain, and their associated blocks.
  ...
  * Note: The service requires exclusive write access to the disk state, and
    ...
    * other parts of the application have a consistent view of the disk state
      and chain fork contexts. For example, the set of in-memory deltas needs to be
...
* **Chain Pruning:**
  * `zebra_state::ChainPruner`: Provides a pruning service that accepts an
    ...
    set of tips, and drops the earliest chain fork contexts in all chains.
  * Note: If there is a chain reorganisation, there may be zero or many
    contexts that are past the reorg limit.
...
* **Chain Tip Updates:**
  * *Not Required:* Since there is only a single chain, the best chain tip is
...
## Chain Fork Context
[chain-fork-context]: #chain-fork-context

The `ChainForkContext` for a block contains the contextual information needed to
verify the next block in the chain fork.
...
* If the context is a field in recent block headers, it may be retrieved via
  the list of recent blocks in the chain fork context.
* Large state, such as unspent transaction outputs (UTXOs), is stored on disk.
  The chain fork context stores deltas from recent blocks, which can be used to
  avoid some disk state queries.

If there are multiple next blocks for a block, they create a new chain fork.
Their chain fork contexts are different, based on the different blocks in each fork.
But they are based on a common context before the fork.

### Chain Fork Context and Network Upgrades
[chain-fork-context-network-upgrades]: #chain-fork-context-network-upgrades

Each Network Upgrade includes some bilateral consensus rule changes. These
consensus rule changes may affect the `ChainForkContext` required to verify
post-activation blocks.

For most blocks, the chain fork updaters create a context with the same fields as
...
* **Genesis:**
  ...
  * The Genesis RecentChainForkUpdater creates a new, empty context, using the
...
* **Heartwood:**
  ...
  * To efficiently verify this field, Zebra can store the list of MMR previous
    subtree roots in the `ChainForkContext` as an `im::Vector`. The number of
  ...
  * Store the full list of previous subtree roots in the `DiskState`. Update by
    replacing the old `DiskState` list with the new `ChainForkContext` set.
  * The Heartwood activation block has an all-zeroes `history_root_hash` field,
    and an empty list of previous subtree roots.

## Best Chain Tip
[best-chain-tip]: #best-chain-tip

The best chain tip determines the set of valid, historical Zcash transactions.
The best chain tip is updated whenever a new block is received:
...
* if the new block forks from a block that is behind a chain tip, the new block
...
* as a tie-breaker, if the main tip is not one of the chain tips with the
   ...
   * Note: we can avoid this edge case by making sure that all verified
     blocks are associated with a chain fork. This ensures they are behind at
     least one tip. (Or the tip itself.)
...
**TODO:**
  - zcashd divergence
    - verification time as a best chain tip tie-breaker
...
Specific
  ...
  - How many blocks do we need to keep as part of each context? Does it vary by
    network upgrade?
  - Describe the fields in the ChainForkContext and DiskState, and how they are updated
