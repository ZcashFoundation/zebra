...
Zebra uses 2 tools for each release. `release-plz` creates and updates the Release PR from `main`; `ZcashFoundation/cargo-release` owns post-merge release planning and reconciliation. The Release PR is the review gate, and its merge commit is the immutable source of the release. [`.github/cargo-release.yml`](../../../.github/cargo-release.yml) is the only source of tag and GitHub Release policy.

### Review the Release PR

After each push to `main`, release-plz opens or updates a Release PR when it finds releasable crate changes. The PR contains the proposed version bumps and release checklist. The PR gate requires every publishable package with a changed version to have a matching, non-empty changelog section.

Changelogs are authored in source PRs rather than generated by release-plz. Final release headings and any required checkpoint, end-of-support height, README, or operational release-note changes must land on `main` before maintainers approve the Release PR.
...
This operation is read-only. It reconstructs the release plan and runs Cargo 1.91's complete multi-package dry-run, then observes crates.io, tags, and the GitHub Release. It does not publish a crate, create a tag, or change a GitHub Release.

Before publication, a green report with `reason: "incomplete"` is expected. It means the plan and dry-run succeeded, while the planned crates, tags, or GitHub Release are correctly absent. Treat a conflict or a failed dry-run as a release blocker.
...
`resume` applies only when the merged Release PR contains `.github/cargo-release.yml`, which marks the Cargo Release cutover. The action reuses the immutable merge commit and current policy. It can repair configured mutable GitHub Release metadata, such as the release name, notes, draft state, or latest selection. It stops on immutable contradictions, including a crate archive with different source provenance, a tag that targets another commit, or a GitHub Release with a conflicting channel.

Do not manually repeat publication or overwrite tags or GitHub Release state. Escalate conflicts and exhausted crates.io retries for maintainer review. When the automated workflow is unavailable and a maintainer authorizes a manual release, follow the [legacy manual release checklist](https://github.com/ZcashFoundation/zebra/blob/main/.github/PULL_REQUEST_TEMPLATE/release-checklist-legacy.md).
