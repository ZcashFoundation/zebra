...
For generated Release PRs, the PR gate requires every package whose version changes to have a matching, non-empty release section in its changelog. The same validation runs again before publication. Maintainers use the embedded checklist to review changelog completeness, version policy, checkpoints, network data, and operational documentation.
