...
For generated Release PRs, the PR gate requires every package whose version changes to have a matching, non-empty release section in its changelog. Before publication, the release workflow repeats that validation and dry-runs the packages selected by release-plz. Maintainers use the embedded checklist to review changelog completeness, version policy, checkpoints, network data, and operational documentation.
