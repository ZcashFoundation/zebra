...
The Release PR is the manual review gate and the immutable source of the release plan. After maintainers review and merge it, native Cargo validates the complete package set and publishes any missing crates in dependency order. Zebra then verifies each published archive against the merge commit, reconciles the release git tags, verifies the published `zebrad` installation, and creates the public `zebrad` GitHub Release last.

For generated Release PRs, the PR gate requires every publishable package whose version changes to have a matching, non-empty release section in its changelog. Before publication, the release workflow reconstructs the same machine-readable plan from the merge commit and runs one Cargo 1.91 dry run containing the complete package set. Maintainers use the embedded checklist to review changelog completeness, version policy, checkpoints, network data, and operational documentation.
...
The release controller is idempotent. If publication, tag creation, installation verification, or GitHub Release creation is interrupted, rerun the **Release** workflow with the merged Release PR number. The rerun uses controller code from current `main` and checks out the selected merge commit as immutable source. It revalidates that source against current release-plan policy before performing missing operations under the same global publication lock. A historical Release PR that no longer satisfies that policy requires maintainer escalation.

Do not manually repeat publication or replace existing release state. Escalate when a published crate has a different VCS commit, a tag points to another commit, or the GitHub Release uses the wrong release channel. An invalid Release PR or exhausted crates.io retries also require escalation. These contradictions require a maintainer to inspect the external state before recovery continues.
