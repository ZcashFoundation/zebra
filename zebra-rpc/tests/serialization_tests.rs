//! Test if the JSON-RPC responses can be deserialized correctly.
//!
//! We want to ensure that users can use this crate to build RPC clients,
//! so this is an integration test to ensure only the public API is accessed.
...
use zebra_rpc::methods::{
    ...
    AddressBalance, GetAddressUtxos, GetBlock, GetBlockChainInfo, GetBlockHash, GetBlockHeader,
    GetBlockHeaderObject, GetBlockHeightAndHash, GetInfo, GetRawTransaction, SentTransactionHash,
};

#[test]
fn test_deserialize_get_address_utxos() -> Result<(), Box<dyn std::error::Error>> {
    let json = r#"{
    "address": "t1ZA68VQUxQwQixpELPGpMNJ4QhvYERJ6bX",
    "txid": "9dcb3dee6e3f35983f6c12b93baa202cdf5ca0bd24276bdc22ed2988835d44de",
    "outputIndex": 1,
    "script": "76a914a7a804b248440f4b490d7180665fa970fcc1997388ac",
    "satoshis": 5881289,
    "height": 2898708
  }
"#;
    let obj: GetAddressUtxos = serde_json::from_str(json)?;

    let (address, txid, output_index, script, satoshis, height) = obj.into_parts();

    let new_obj =
        GetAddressUtxos::from_parts(address, txid, output_index, script, satoshis, height);
...
#[test]
fn test_get_block_template() -> Result<(), Box<dyn std::error::Error>> {
