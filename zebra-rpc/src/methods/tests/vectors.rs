...
async fn rpc_proposal_rejects_invalid_subsidy_before_verification() {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        ...
        for (tip_height, tip_hash, subsidy, should_verify, forged_height) in [
            (height.previous().unwrap(), parent_hash, expected_block_subsidy, false, false),
            (height.previous().unwrap(), Hash([2; 32]), scheduled, false, false),
            (height, parent_hash, scheduled, false, false),
            (height.previous().unwrap(), parent_hash, scheduled, true, false),
            // The header has the live parent, but the coinbase claims a pre-reissuance height.
            (height, parent_hash, scheduled, false, true),
        ] {
