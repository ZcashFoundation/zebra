...
async fn test_rpc_response_data_for_network(network: &Network) {
    ...
    settings.bind(|| insta::assert_json_snapshot!("getrawtransaction_verbosity=0".to_string(), rsp));
    ...
    let (rsp, _) = futures::join!(rpc_req, mempool_req);
    settings.bind(|| insta::assert_json_snapshot!("getrawtransaction_verbosity=1".to_string(), rsp));
    mempool.expect_no_requests().await;
    ...
    let (rsp, _) = futures::join!(rpc_req, mempool_req);
    settings.bind(|| insta::assert_json_snapshot!("getrawtransaction_unknown_txid".to_string(), rsp));
    mempool.expect_no_requests().await;
    ...
    let rsp = rpc
        ...
        .await;
    settings.bind(|| insta::assert_json_snapshot!("getrawtransaction_invalid_txid".to_string(), rsp));
    mempool.expect_no_requests().await;
