...
fn combine_coinbase_outputs(
    ...
    like_zcashd: bool,
) -> Vec<(Amount<NonNegative>, transparent::Address)> {
    // Combine all the funding streams with the miner reward.
    if like_zcashd {
        use FundingStreamReceiver::*;

        // zcashd uses the order: [miner, ECC, MG, ZF], so we apply that custom order here.
        let mut coinbase_outputs = vec![(miner_reward, miner_address)];

        for funding_stream in [Ecc, MajorGrants, ZcashFoundation] {
            if let Some((amount, address)) = funding_streams.remove(&funding_stream) {
                coinbase_outputs.push((amount, address));
            }
        }

        // Sort remaining funding streams by amount then address.
        //
        // (There aren't any of these yet, but we don't want to break consensus if they are added.)
        let mut remaining_funding_streams: Vec<(Amount<NonNegative>, transparent::Address)> =
            funding_streams
                .into_iter()
                .map(|(_receiver, (amount, address))| (amount, address))
                .collect();

        // Since sort_by() uses a stable sort, we can just sort by address first, then amount,
        // and any equal amounts will be in address order.
        remaining_funding_streams.sort_by_key(|(_amount, address)| address.to_string());
        remaining_funding_streams.sort_by_key(|(amount, _address)| *amount);

        coinbase_outputs.extend(remaining_funding_streams);

        coinbase_outputs
    } else {
        // Zebra sorts by amount then address.
        let mut coinbase_outputs: Vec<(Amount<NonNegative>, transparent::Address)> =
            funding_streams
                .into_iter()
                .map(|(_receiver, (amount, address))| (amount, address))
                .collect();
        ...
        coinbase_outputs.sort_by_key(|(_amount, address)| address.to_string());
        coinbase_outputs.sort_by_key(|(amount, _address)| *amount);
