...
fn coinbase_cache_retains_both_fake_and_real_fee_entries() {
    ...
    cache.select(next_height, next_subsidy);
    assert!(cache.get(next_height, next_subsidy, zero_fee).is_none());
...
#[test]
fn coinbase_cache_discards_late_old_subsidy_builds() {
    use super::CoinbaseCache;

    let network = Network::Mainnet;
    let height = Height(2_000_000);
    let subsidy = scheduled_block_subsidy(height, &network).unwrap();
    let other_subsidy = (subsidy + Amount::try_from(1).unwrap()).unwrap();
    let fee = Amount::zero();
    let miner_params = MinerParams::from(
        Address::decode(
            &network,
    ...
    let build = |subsidy| {
        TransactionTemplate::new_coinbase(&network, height, &miner_params, subsidy, fee).unwrap()
    };
    let cache = CoinbaseCache::default();
    cache.select(height, subsidy);
    assert!(cache.get(height, subsidy, fee).is_none());
    let old_coinbase = build(subsidy);
    cache.select(height, other_subsidy);
    assert!(cache.get(height, other_subsidy, fee).is_none());
    let current_coinbase = build(other_subsidy);
    cache.store(height, other_subsidy, fee, current_coinbase.clone());
    cache.store(height, subsidy, fee, old_coinbase);
    assert!(cache.get(height, subsidy, fee).is_none());
    assert_eq!(
        cache.get(height, other_subsidy, fee),
