    ...
    fn value_pool_is_updated(
        ...
        for block in finalized_blocks {
            ...
            if block.height > block::Height(0) {
                ...
                let block_value_pool = &block.block.chain_value_pool_change(utxos, DeferredPoolBalanceChange::zero(), &network)?;
        ...
        for block in non_finalized_blocks {
            ...
            let block_value_pool = &block.block.chain_value_pool_change(&transparent::utxos_from_ordered_utxos(utxos), DeferredPoolBalanceChange::zero(), &network)?;
