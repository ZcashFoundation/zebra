...
fn check_sprout_anchors() {
    ...
    for tx in block1.block.transactions.iter() {
        if tx.has_sprout_joinsplit_data() {
            println!("tx has JoinSplits");

            for joinsplit in tx.sprout_groth16_joinsplits() {
                println!("anchor in tx: {:?}", joinsplit.anchor);
            }
        }
    }
...
fn check_sapling_anchors() {
    ...
    for tx in block1.block.transactions.iter() {
        if tx.has_sapling_shielded_data() {
            println!("tx has shielded data");

            for anchor in tx.sapling_anchors() {
                println!("anchor in tx: {:?}", anchor);
            }
        }
    }
