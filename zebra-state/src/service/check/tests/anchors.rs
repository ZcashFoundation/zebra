...
fn check_sprout_anchors() {
    ...
    assert_eq!(
        validate_and_commit_non_finalized(&mut state.disk, &mut state.mem, state.network, block_2),
        Ok(())
    );
}
...
fn check_sapling_anchors() {
    ...
    assert_eq!(
        validate_and_commit_non_finalized(&mut state.disk, &mut state.mem, state.network, block2),
        Ok(())
    );
}
