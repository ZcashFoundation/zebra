...
proptest! {
    ...
    fn accept_distinct_arbitrary_orchard_nullifiers_in_one_block(
        authorized_action in TypeNameToDebug::<orchard::AuthorizedAction<TxV5>>::arbitrary(),
        orchard_shielded_data in TypeNameToDebug::<orchard::ShieldedData<TxV5>>::arbitrary(),
        use_finalized_state in any::<bool>(),
    ) {
        let _init_guard = zebra_test::init();
    ...
    fn reject_duplicate_orchard_nullifiers_in_transaction(
        authorized_action1 in TypeNameToDebug::<orchard::AuthorizedAction<TxV5>>::arbitrary(),
        mut authorized_action2 in TypeNameToDebug::<orchard::AuthorizedAction<TxV5>>::arbitrary(),
        orchard_shielded_data in TypeNameToDebug::<orchard::ShieldedData<TxV5>>::arbitrary(),
    ) {
        let _init_guard = zebra_test::init();
    ...
    fn reject_duplicate_orchard_nullifiers_in_block(
        authorized_action1 in TypeNameToDebug::<orchard::AuthorizedAction<TxV5>>::arbitrary(),
        mut authorized_action2 in TypeNameToDebug::<orchard::AuthorizedAction<TxV5>>::arbitrary(),
        orchard_shielded_data1 in TypeNameToDebug::<orchard::ShieldedData<TxV5>>::arbitrary(),
        orchard_shielded_data2 in TypeNameToDebug::<orchard::ShieldedData<TxV5>>::arbitrary(),
    ) {
        let _init_guard = zebra_test::init();
...
fn transaction_v5_with_orchard_shielded_data(
    orchard_shielded_data: impl Into<Option<orchard::ShieldedData<TxV5>>>,
    authorized_actions: impl IntoIterator<Item = orchard::AuthorizedAction<TxV5>>,
) -> Transaction {
