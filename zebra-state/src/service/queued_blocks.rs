...
/// A checkpoint verified block queue, and its corresponding [`Result`] channel.
pub type QueuedCheckpointVerified = (
    CheckpointVerifiedBlock,
    oneshot::Sender<Result<block::Hash, BoxError>>,
);

/// A semantically verified block queue, and its corresponding [`Result`] channel.
...
impl SentHashes {
    ...
    /// Stores the checkpoint verified `block`'s hash, height, and UTXOs into finalized, so they can be used to check if a
    /// block or UTXO is available in the state.
    ///
    /// Used for checkpoint verified blocks close to the final checkpoint, so state non-finalized blocks can look up
    /// their UTXOs.
    ///
    /// Assumes that blocks are added in the order of their height between `finish_batch` calls
    /// for efficient pruning.
