...
impl NonFinalizedState {
        ...
        for chain in self.chain_iter() {
            if !chain.contains_block_hash(hash) {
                ...
                for (i, block) in chain
                    .child_blocks(&chain.non_finalized_root_height())
                    .iter()
                    .enumerate()
                {
                    info!("  {}: {}", i, block.hash);
    ...
    pub fn crosslink_finalize(
        ...
        if let Some(chain_containing_finalized_block) =
            self.find_chain(|chain| chain.height_by_hash(hash).is_some())
        {
            ...
            Some(
                chain_containing_finalized_block
                    .blocks
                    .range(..=height)
                    .map(|(_h, b)| b.clone())
                    .collect(),
            ...
            warn!(
                "crosslink finalize: couldn't find block in any chain: {}",
                hash
            );
            None
        }
