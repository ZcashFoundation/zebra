...
//!
//! This column family stores known-hash chunks (the v2 chunk byte format) that
//! have been verified against the SHA-256 constants pinned in `zebra-chain`
//! before being persisted: the chunk *hashes* stay in `zebra-chain` as the
//! trust root, but the verified chunk *bytes* live here in the state database
//! rather than in shipped asset files.
//!
...
impl ZebraDb {
    ...
    /// consumers don't need it: they generate chunks from state on demand rather
    /// than reading the download cache.
    ...
    ///
    /// The bytes are the v2 chunk format as they were downloaded and verified
    /// before being written via [`ZebraDb::write_known_hash_chunk`]. Returns
    /// `None` when the column family is not open (read-only DB without it) or
    /// has no chunk at `index`.
