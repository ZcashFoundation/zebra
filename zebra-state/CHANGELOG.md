...
- `ReadRequest`, which is not `#[non_exhaustive]`, gains two variants
  (`AnyChainSaplingTree` and `AnyChainOrchardTree`). Consumers that match on
  `ReadRequest` exhaustively must handle the new variants.
