...
static ADD_BLOCK_TRANSCRIPT_MAINNET: Lazy<Vec<(Request, Response)>> = Lazy::new(|| {
    let block: Arc<_> =
...
static ADD_BLOCK_TRANSCRIPT_TESTNET: Lazy<Vec<(Request, Response)>> = Lazy::new(|| {
    let block: Arc<_> = Block::zcash_deserialize(&zebra_test::vectors::BLOCK_TESTNET_10_BYTES[..])
        .unwrap()
        .into();
    let hash = block.as_ref().into();
    vec![
...
static GET_TIP_TRANSCRIPT_MAINNET: Lazy<Vec<(Request, Response)>> = Lazy::new(|| {
    let block0: Arc<_> =
...
static GET_TIP_TRANSCRIPT_TESTNET: Lazy<Vec<(Request, Response)>> = Lazy::new(|| {
    let block0: Arc<_> =
        Block::zcash_deserialize(&zebra_test::vectors::BLOCK_TESTNET_GENESIS_BYTES[..])
            .unwrap()
            .into();
    let block1: Arc<_> = Block::zcash_deserialize(&zebra_test::vectors::BLOCK_TESTNET_10_BYTES[..])
        .unwrap()
        .into();
    let hash0 = block0.as_ref().into();
    let hash1 = block1.as_ref().into();
