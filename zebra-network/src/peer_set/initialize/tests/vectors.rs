...
async fn crawler_peer_limit_zero_connect_panic() {
    ...
    assert!(
        // `Err(Empty)` means that no peers are available, and the sender has not been dropped.
        // `Err(Closed)` means that no peers are available, and the sender has been dropped.
        peer_result.is_err(),
