...
use crate::{
    ...
    peer_set::{
        ...
        stall_tracker::FIND_RESPONSE_STALL_THRESHOLD, PeerSet,
    },
    protocol::external::{
...
fn peer_set_waits_for_busy_block_peer_without_refusal() {
    ...
    runtime.block_on(async move {
        ...
        let service = retry_busy_inventory(tower::buffer::Buffer::new(peer_set, 4));
        ...
        request
            .tx
            .send(Err(PeerError::NotFoundResponse(vec![hash]).into()))
            .unwrap();
        assert!(
        ...
        request.tx.send(Ok(Response::Blocks(vec![]))).unwrap();
        response
            .await
...
fn peer_set_block_failure_allows_responsive_fallback() {
    for cancel in [false, true] {
        ...
        runtime.block_on(async move {
            ...
            // An explicit advertisement gives the failing peer a chance to recover.
            guard
                .inventory_sender()
                .as_mut()
                .unwrap()
