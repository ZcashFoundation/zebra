...
/// Tests that an unusable non-empty `FindBlocks` response preserves stalls.
///
/// A peer must not evade the stall threshold by returning hashes that do not
/// help the response consumer advance synchronization.
#[test]
fn unusable_find_blocks_response_does_not_clear_stalls() {
    let peer_version = Version::min_specified_for_upgrade(&Network::Mainnet, NetworkUpgrade::Nu6_2);
    let peer_versions = PeerVersions {
        peer_versions: vec![peer_version],
    };
    ...
    runtime.block_on(async move {
        ...
        let client_request = handle
            ...
            .expect("peer received the request");
        let _ = client_request.tx.send(Ok(Response::BlockHashes {
            hashes: vec![block::Hash([1; 32])],
            feedback: None,
        }));
        let response = response_fut.await.expect("response received");
        let Response::BlockHashes { feedback, .. } = response else {
            panic!("FindBlocks returns block hashes");
        };
        feedback
            .expect("a tracked FindBlocks response includes classification feedback")
            .mark_stalled();
        ...
        assert!(
            !handle.wants_connection_heartbeats(),
            "peer should be disconnected after an unusable non-empty response reaches the stall \
            threshold",
        );
    });
}

/// Tests that cancelling a `FindBlocks` response does not block later stall outcomes.
///
/// A routed response future can be dropped before the peer responds. Its pending
/// outcome must not remain at the front of the peer's ordered response queue.
#[test]
fn cancelled_find_blocks_response_does_not_block_stalls() {
    let peer_version = Version::min_specified_for_upgrade(&Network::Mainnet, NetworkUpgrade::Nu6_2);
    let peer_versions = PeerVersions {
        peer_versions: vec![peer_version],
    };
    ...
    runtime.block_on(async move {
        ...
        let peer_ready = peer_set.ready().await.expect("peer set is ready");
        let cancelled_response = peer_ready.call(Request::FindBlocks {
            known_blocks: vec![],
            stop: None,
        });
        handle
            .try_to_receive_outbound_client_request()
            .request()
            .expect("peer received the cancelled request");
        ...
        let client_request = handle
            .try_to_receive_outbound_client_request()
            .request()
            .expect("peer received the request after cancellation");
        let _ = client_request.tx.send(Ok(Response::BlockHashes {
            hashes: vec![],
            feedback: None,
        }));
        ...
        assert!(
            !handle.wants_connection_heartbeats(),
            "peer should be disconnected when cancellation unblocks the threshold stall",
        );
    });
}
...
fn busy_sidecar_receives_queued_block_gossip() {
    ...
    runtime.block_on(async move {
        ...
        let _ = find_blocks_request.tx.send(Ok(Response::BlockHashes {
            hashes: vec![],
            feedback: None,
        }));
