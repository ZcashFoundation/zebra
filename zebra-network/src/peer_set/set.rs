    ...
    C: ChainTip,
{
    ...
    /// Routes inventory to a ready advertiser, then a ready peer not known to be missing it.
    ///
    /// Block fallback prefers responsive peers, then peers advertising `NODE_NETWORK`. A missing
    /// advertisement does not exclude pruned peers: the inventory registry is deliberately lossy.
    ///
    /// If only busy candidates remain, the buffered network retries internally without spending
    /// the caller's missing-block retry budget. Truly missing inventory is refused immediately.
    fn route_inv(
        &mut self,
        req: Request,
        hash: InventoryHash,
        ...
        let advertising_peer_list = self
            ...
            .copied()
            .collect();
        let mut peer = self.select_p2c_peer_from_list(&advertising_peer_list);
        let missing_peer_list = self
            .inventory_registry
            .missing_peers(hash)
            .copied()
            .collect();
        ...
        let is_advertised = self
            ...
            .next()
            .is_some();
        // Compute this before dispatch so the selected peer cannot count as its own alternative.
        let retry_busy = is_block && self.busy_peer_might_have(&missing_peer_list, !is_advertised);

        if peer.is_none() {
            let mut candidates: HashSet<_> = self
                .ready_services
                .keys()
                .filter(|key| !missing_peer_list.contains(key))
                .copied()
                .collect();
            if is_block {
                // Failed requests (including cancellations) must not give a serving peer an
                // exclusive preference over responsive pruned peers. This is not missing inventory.
                let responsive: HashSet<_> = candidates
                    .iter()
        ...
        if let Some(peer) = peer {
            ...
            return async move {
                match fut.await {
                    // A pruned peer can reject a historic block while its serving alternative is
                    // busy. Retry routing internally, after the missing-inventory update is consumed.
                    Err(error) if retry_busy => {
                        tracing::debug!(?hash, ?peer, %error, "retrying inventory with busy alternatives");
        ...
        if retry_busy {
            return async { Err(BoxError::from(InventoryBusy)) }.boxed();
        }
