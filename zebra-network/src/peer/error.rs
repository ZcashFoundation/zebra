...
/// A shareable [`PeerError`] that keeps its machine-readable kind.
#[derive(Debug, Clone)]
pub struct SharedPeerError {
    source: Arc<TracedError<PeerError>>,
    error_kind: PeerErrorKind,
    kind: Cow<'static, str>,
    missing_inventory: Option<Arc<[InventoryHash]>>,
}
    ...
    PeerError: From<E>,
{
    fn from(source: E) -> Self {
        let peer_error = PeerError::from(source);
        let error_kind = peer_error.error_kind();
        let kind = peer_error.kind();
        let missing_inventory = peer_error
            .missing_inventory()
            .map(|inventory| Arc::from(inventory.to_vec()));

        Self {
            source: Arc::new(TracedError::from(peer_error)),
            error_kind,
            kind,
            missing_inventory,
        }
...
impl fmt::Display for SharedPeerError {
...
impl SharedPeerError {
    ...
    /// Returns the machine-readable peer error kind.
    pub fn error_kind(&self) -> PeerErrorKind {
        self.error_kind
    }

    /// Returns true when the peer or inventory registry did not have requested inventory.
    pub fn is_inventory_not_found(&self) -> bool {
        matches!(
            self.error_kind(),
            PeerErrorKind::NotFoundResponse | PeerErrorKind::NotFoundRegistry
        )
    }

    /// Returns true when the missing inventory status was generated by the local registry.
    pub fn is_inventory_not_found_by_registry(&self) -> bool {
        self.error_kind() == PeerErrorKind::NotFoundRegistry
    }

    /// Returns the inventory items that were explicitly reported missing.
    pub fn missing_inventory(&self) -> &[InventoryHash] {
        self.missing_inventory.as_deref().unwrap_or(&[])
    }

    /// Returns a debug-formatted string describing the inner [`PeerError`].
    pub fn inner_debug(&self) -> String {
        format!("{:?}", self.source.as_ref())
    }
}

/// The machine-readable kind of a [`PeerError`].
...
impl PeerError {
    ...
    pub fn error_kind(&self) -> PeerErrorKind {
        match self {
            PeerError::ConnectionClosed => PeerErrorKind::ConnectionClosed,
            PeerError::ConnectionDropped => PeerErrorKind::ConnectionDropped,
            PeerError::ClientDropped => PeerErrorKind::ClientDropped,
            PeerError::ClientCancelledHeartbeatTask => PeerErrorKind::ClientCancelledHeartbeatTask,
            PeerError::HeartbeatTaskExited(_) => PeerErrorKind::HeartbeatTaskExited,
            PeerError::ConnectionTaskExited => PeerErrorKind::ConnectionTaskExited,
            PeerError::ConnectionSendTimeout => PeerErrorKind::ConnectionSendTimeout,
            PeerError::ConnectionReceiveTimeout => PeerErrorKind::ConnectionReceiveTimeout,
            PeerError::Serialization(_) => PeerErrorKind::Serialization,
            PeerError::DuplicateHandshake => PeerErrorKind::DuplicateHandshake,
            PeerError::Overloaded => PeerErrorKind::Overloaded,
            PeerError::NoReadyPeers => PeerErrorKind::NoReadyPeers,
            PeerError::InboundTimeout => PeerErrorKind::InboundTimeout,
            PeerError::ServiceShutdown => PeerErrorKind::ServiceShutdown,
            PeerError::NotFoundResponse(_) => PeerErrorKind::NotFoundResponse,
            PeerError::NotFoundRegistry(_) => PeerErrorKind::NotFoundRegistry,
        }
    }

    /// Returns the inventory items that were explicitly reported missing.
    pub fn missing_inventory(&self) -> Option<&[InventoryHash]> {
        match self {
            PeerError::NotFoundResponse(inventory) | PeerError::NotFoundRegistry(inventory) => {
                Some(inventory)
            }
            _ => None,
        }
