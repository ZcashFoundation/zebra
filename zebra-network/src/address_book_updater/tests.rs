...
async fn address_book_service_serves_all_request_variants() {
    ...
    let gossiped_change = |address_number: usize| {
        ...
        MetaAddr::new_gossiped_meta_addr(addr.into(), PeerServices::NODE_NETWORK, DateTime32::now())
            .new_gossiped_change()
    ...
    assert!(
        matches!(response, AddressBookResponse::Updated(Some(_))),
        "a new gossiped change should update the address book: {response:?}",
    );
    ...
    let response = address_book_service
        ...
        .expect("service should be running");
    assert!(
        matches!(response, AddressBookResponse::NextReconnectPeer(Some(_))),
        "gossiped peers should be available for reconnection: {response:?}",
    );
}
...
async fn concurrent_next_reconnect_peer_requests_return_distinct_peers() {
    ...
    let requests = (0..TEST_PEER_COUNT).map(|_| {
        let handle = address_book_service.clone();
        async move { handle.oneshot(AddressBookRequest::NextReconnectPeer).await }
    });
