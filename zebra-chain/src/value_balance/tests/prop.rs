...
proptest! {
        ...
        value_balance2 in any::<ValueBalance<NegativeAllowed>>())
    {
        ...
        let nsm_reserve = value_balance1.nsm_reserve + value_balance2.nsm_reserve;

        match (transparent, sprout, sapling, orchard, deferred, ironwood, nsm_reserve) {
            (Ok(transparent), Ok(sprout), Ok(sapling), Ok(orchard), Ok(deferred), Ok(ironwood), Ok(nsm_reserve)) => prop_assert_eq!(
            ...
            _ => prop_assert!(
                matches!(
                    ...
                    Err(ValueBalanceError::Transparent(_)
                        ...
                        | ValueBalanceError::NsmReserve(_))
                )
            ),
        }
    }
    ...
    /// Earlier versions of [`ValueBalance`] had 32 bytes (no `deferred`), then 40 bytes (no
    /// `ironwood`), then 48 bytes (no `nsm_reserve`), compared to the current 56 bytes. It's
    /// possible to correctly instantiate the current version from any legacy format, with the
    /// missing trailing balances defaulting to zero, so we test that Zebra can still deserialize
    /// all of them.
    #[test]
