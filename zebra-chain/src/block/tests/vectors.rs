...
#[test]
fn genesis_outputs_do_not_reduce_nsm_seed() {
    ...
    for value in [0, 1, MAX_MONEY] {
        ...
        for network in [&network, &Network::Mainnet] {
            ...
            let expected_reserve = if Nu7.activation_height(network) == Some(Height(1)) {
                scheduled_block_subsidy(Height::MIN, network).unwrap()
            } else {
                Amount::zero()
            };
            assert_eq!(pools.nsm_reserve_amount(), expected_reserve);
            assert_eq!(pools.total().unwrap(), expected_reserve);
        }
    }
}
