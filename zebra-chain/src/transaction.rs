...
pub enum Transaction {
    ...
    // TODO: let's just get a v6 extensible tx format impl here and add this as a bundle and design it
    //       so that it can be moved to librustzcash but can still be extended without direct edits, i.e.
    //       we can add bundles without editing librustzcash
...
impl Transaction {
    ...
    pub fn zip233_amount(&self) -> Amount<NonNegative> {
        match self {
            ...
            | Transaction::V4 { .. }
            | Transaction::V5 { .. }
            | Transaction::VCrosslink { .. } => Amount::zero(),
            #[cfg(all(zcash_unstable = "nu7", feature = "tx_v6"))]
