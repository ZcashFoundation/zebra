...
use crate::{
    ...
    orchard::{tx_version, Action, AuthorizedAction},
...
proptest! {
    ...
    #[test]
    fn authorized_action_size_is_small_enough(authorized_action in <AuthorizedAction<tx_version::V5>>::arbitrary_with(())) {
        ...
        prop_assert!(serialized_len as u64 == AuthorizedAction::<tx_version::V5>::AUTHORIZED_ACTION_SIZE)
    }

    /// Verify trusted preallocation for `AuthorizedAction` and its split fields
    #[test]
    fn authorized_action_max_allocation_is_big_enough(authorized_action in <AuthorizedAction<tx_version::V5>>::arbitrary_with(())) {
        let (action, spend_auth_sig) = authorized_action.into_parts();
        ...
        prop_assert!((smallest_disallowed_serialized_len as u64) / AuthorizedAction::<tx_version::V5>::ACTION_SIZE *
            AuthorizedAction::<tx_version::V5>::AUTHORIZED_ACTION_SIZE >= MAX_BLOCK_BYTES);
        prop_assert!((largest_allowed_serialized_len as u64) / AuthorizedAction::<tx_version::V5>::ACTION_SIZE *
            AuthorizedAction::<tx_version::V5>::AUTHORIZED_ACTION_SIZE <= MAX_BLOCK_BYTES);

        // Check the serialization limits for `Action`
        prop_assert!(((smallest_disallowed_vec_len - 1) as u64) == Action::<tx_version::V5>::max_allocation());
        prop_assert!((largest_allowed_vec_len as u64) == Action::<tx_version::V5>::max_allocation());
