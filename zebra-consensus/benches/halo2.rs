...
use zebra_consensus::halo2::{Item, VERIFYING_KEY_V5_PRE_NU6_2};
...
fn extract_halo2_items_from_blocks() -> Vec<Item> {
    ...
    for bytes in zebra_test::vectors::MAINNET_BLOCKS.values() {
        ...
        for tx in &block.transactions {
            ...
            // below, which pass `VERIFYING_KEY_V5_PRE_NU6_2`).
...
fn bench_halo2_verify(c: &mut Criterion) {
    ...
    group.bench_function("single bundle", |b| {
        ...
        b.iter(|| {
            assert!(item.clone().verify_single(&VERIFYING_KEY_V5_PRE_NU6_2));
        })
    });
