...
- The per-arch image digest can still drift across days, because `adduser` writes the account creation date into `/etc/shadow`. The binary stays reproducible; pinning that date is a possible follow-up.
