...
- Sign and attest release images in the `merge` job, gated to production releases
  (main/dispatch stay unsigned): a signed SLSA provenance attestation
  (`actions/attest-build-provenance`), a signed SBOM (`actions/attest-sbom`), and a
  Cosign keyless signature, all on the final multi-arch digest. A verify step then
  fails the release if the signature or attestations do not verify, pinning the
  signer with `gh attestation verify --signer-workflow`.
...
- Release images carry signed provenance, a signed SBOM, and a Cosign signature,
  verifiable with `gh attestation verify` or `cosign verify` and checked in CI before
  the release completes. This is signed (L2-grade) provenance: formal SLSA L3
  additionally needs provenance generated by a separate trusted workflow the build
  cannot influence, tracked separately.
