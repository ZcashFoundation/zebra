...
#
# This function works with a *copy* of the config file, avoiding
# "Device or resource busy" errors with `sed -i` on mounted volumes.
#
# It prioritizes user-mounted config files, then the default config,
# and applies environment variable overrides *last*.
prepare_conf_file() {
  # Base configuration
